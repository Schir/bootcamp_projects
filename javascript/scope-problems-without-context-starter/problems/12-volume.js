...
const recVolume = height =>{
  let arr=[height];
  return function1 = width =>{
    //I should really add an if(arr.length < 3) check here. this seems like it's probably terrible for memory optimization.
    arr.push(width);
    return arr.length>2? arr[0]*arr[1]*arr[2]: function1;
  }
}
